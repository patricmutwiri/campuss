...
//                echo nl2br(str_replace('#__','jos_',$query));
	...
	public function getModules($limitstart, $key = null) {
		...
		if ($key != '')
		{
			$query .= 'AND a.title LIKE "%'. (string) $key . '%" ';
		}

		$query .= "GROUP BY a.id, a.title, a.note, a.position, a.module, a.language,a.checked_out,a.checked_out_time, a.published, a.access, a.ordering,l.title, uc.name, ag.title, e.name,l.lang_code, uc.id, ag.id, mm.moduleid, e.element, a.publish_up, a.publish_down,e.enabled";
		
		$db->setQuery($query);
		$total_modules  = $db->loadObjectList();
		
		$modules = $db->setQuery($query, $limitstart, 20);
		$modules = $db->loadObjectList();	
		
		return array(
			'total_modules' => count($total_modules),
			'modules' => $modules,
			'key' => $key				
		);
	}
