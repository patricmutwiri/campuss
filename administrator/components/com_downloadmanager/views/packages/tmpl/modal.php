            ...
            <?php foreach($result as $post) {
                ...
                <tr valign="top" class="alternate author-self status-inherit" id="post-<?php echo $post->id; ?>">
                    ...
                    <td class="media column-media">
                        <a href="javascript:void(0)" onclick="if (window.parent) window.parent.<?php echo $this->escape($_REQUEST['function']);?>('<?php echo $post->id; ?>', '<?php echo $this->escape(addslashes($post->title)); ?>', '<?php echo $this->escape($post->id); ?>', null, '<?php echo $this->escape(DownloadManagerHelperRoute::getPackageRoute($post->id, $post->id, 'en')); ?>', '<?php echo $this->escape('en'); ?>', null);">
                            <?php echo $post->title; ?></a>
                    </td>
                    <td class="actions"><a href='#' class='selectp btn btn-info btn-xs pull-right' rel='<?php echo $post->id; ?>' title="Select Package" ><i class="fa fa-plus"></i></a></td>
