...
function dm_categories_checkboxes($params){
    $db = JFactory::getDBO();
    $html  = "";
    ...
    $query
        ...
        ->order('cid desc');
    if(isset( $params['parent'] )) $query->where("parent = '{$params['parent']}'");
    $db->setQuery($query);
    $result = $db->loadObjectList();
    foreach($result as $cat){
        $checked = ($cat->cid == $params['selected'])?'checked=checked':'';
        $html .= "<li><label class='eden-radio'><input $checked type='radio' name='{$params['name']}' value='{$cat->cid}' /><span class='fa fa-check'></span> {$cat->name}</label>";
        $params['parent'] = $cat->cid;
        $html .= dm_categories_checkboxes($params);
        $html .= "</li>";
    }

    $html = $html != ""?"<ul>$html</ul>":$html;
    if( isset($params['echo']) && $params['echo'] == 1 ) echo $html;
    else return $html;
}
...
function dm_download_file($filepath, $filename, $speed = 0, $resume_support = 1, $extras = array())
{
    ...
    header("Robots: none");
    header ('Content-Description: File Transfer') ;
    if(strpos($_SERVER['HTTP_USER_AGENT'],"Safari") && !isset($_REQUEST['play']))
