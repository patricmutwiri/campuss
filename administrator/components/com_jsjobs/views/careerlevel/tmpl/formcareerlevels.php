...
// for joomla 1.6
    Joomla.submitbutton = function (task) {
        if (task == '') {
            ...
            if (task == 'careerlevel.savejobcareerlevelsave' || task == 'careerlevel.savejobcareerlevelandnew' || task == 'careerlevel.savejobcareerlevel') {
...
<div id="jsjobs-wrapper">
    ...
    <div id="jsjobs-content">
        <div id="jsjobs-heading"><a id="backimage" href="index.php?option=com_jsjobs&c=careerlevel&view=careerlevel&layout=careerlevels"><img src="components/com_jsjobs/include/images/back-icon.png" alt="<?php echo JText::_('Back');?>" ></a>
        <?php if (isset($this->application->id)){ ?>
        <span id="heading-text"><?php echo JText::_('Edit Career Level'); ?></span>
        <?php }else{ ?>
        <span id="heading-text"><?php echo JText::_('Form Career Level'); ?></span>
         <?php } ?>
        </div>
...
?>/></span> <label class="js-publish" for="status" ><?php echo JText::_('Publish'); ?></label>
                ...
                <input type="hidden" name="task" value="careerlevel.savejobcareerlevel" />
                <input type="hidden" name="option" value="<?php echo $this->option; ?>" />
                <?php echo JHTML::_( 'form.token' ); ?>        
            </div>
            <div class="js-buttons-area">
                <a class="js-btn-cancel" href="index.php?option=com_jsjobs&c=careerlevel&view=careerlevel&layout=careerlevels"><?php echo JText::_('Cancel'); ?></a>
                <input type="submit" class="js-btn-save" name="submit_app" onclick="return validate_form(document.adminForm)" value="<?php echo JText::_('Save Career Level'); ?>" />
            </div>
        </form>
    </div>
</div>
