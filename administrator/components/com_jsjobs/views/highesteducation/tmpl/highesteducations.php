...
<div id="jsjobs-wrapper">
    ...
    <div id="jsjobs-content">
        <div id="jsjobs-heading"><a id="backimage" href="index.php?option=com_jsjobs&c=jsjobs&view=jsjobs&layout=controlpanel"><img src="components/com_jsjobs/include/images/back-icon.png" alt="<?php echo JText::_('Back');?>" ></a><span id="heading-text"><?php echo JText::_('Highest Education'); ?></span></div>
      <form action="index.php" method="post" name="adminForm" id="adminForm">
          <div id="jsjobs_filter_wrapper">           
              ...
              <span class="jsjobs-filter"><button class="js-button" onclick="document.getElementById('searchstatus').value = '';document.getElementById('searchtitle').value = ''; this.form.submit();"><?php echo JText::_('Reset'); ?></button></span>
          </div> 
            <?php if(!empty($this->items)){ ?>
                <table class="adminlist" border="0" id="js-table">
                    ...
                    for ($i = 0, $n = count($this->items); $i < $n; $i++) {
                        ...
                        $link = JFilterOutput::ampReplace('index.php?option=' . $this->option . '&task=highesteducation.editjobhighesteducation&cid[]=' . $row->id);
                        ?>
                        <tr valign="top" class="<?php echo "row$k"; ?>">
                            ...
                            <td align="center">
                                ...
                                <?php } else { ?>
                                    <a href="index.php?option=com_jsjobs&c=common&task=makedefault&for=heighesteducation&id=<?php echo $row->id; ?>">
                            ...
                            <td align="center">
                                ...
                                $status_link = JFilterOutput::ampReplace('index.php?option='.$this->option.'&task=common.makePublishUnpublish&for=heighesteducation&status='.$status_value.'&cid[]='.$row->id);
                                ?>
                            ...
                            <td align="center">
                                <?php if ($i != 0 || $this->pagination->pagesCurrent > 1) { ?>
                                    <a href="index.php?option=com_jsjobs&c=common&task=defaultorderingdown&for=heighesteducation&id=<?php echo $row->id; ?>">
                                ...
                                <?php if ($i < $n - 1 || $this->pagination->pagesCurrent < $this->pagination->pagesTotal) { ?>
                                    <a href="index.php?option=com_jsjobs&c=common&task=defaultorderingup&for=heighesteducation&id=<?php echo $row->id; ?>">
...
<?php }else{ JSJOBSlayout::getNoRecordFound(); } ?>            
                <input type="hidden" name="layout" value="highesteducations" />
                <input type="hidden" name="view" value="highesteducation" />
                <input type="hidden" name="c" value="highesteducation" />
