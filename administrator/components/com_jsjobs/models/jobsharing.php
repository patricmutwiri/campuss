...
class JSJobsModelJobsharing extends JSModel {
    ...
    function serverTask($jsondata, $fortask) {
        ...
        switch ($fortask) {
            ...
            case "storeresume";
                ...
                $return_value = $this->performTaskJobSharingService($encoded_server_json_data_array, $url);
                break;
            case "storeresumeaddresses";
                $url = $sitepath . 'index.php?r=Resume/saveresumeaddresses';
                $return_value = $this->performTaskJobSharingService($encoded_server_json_data_array, $url);
                break;
            case "storeresumeinstitutes";
                $url = $sitepath . 'index.php?r=Resume/saveresumeinstitutes';
                $return_value = $this->performTaskJobSharingService($encoded_server_json_data_array, $url);
                break;
            case "storeresumeemployers";
                $url = $sitepath . 'index.php?r=Resume/saveresumeemployers';
                $return_value = $this->performTaskJobSharingService($encoded_server_json_data_array, $url);
                break;
            case "storeresumereferences";
                $url = $sitepath . 'index.php?r=Resume/saveresumereferences';
                $return_value = $this->performTaskJobSharingService($encoded_server_json_data_array, $url);
                break;
            case "storeresumelanguages";
                $url = $sitepath . 'index.php?r=Resume/saveresumelanguages';
                $return_value = $this->performTaskJobSharingService($encoded_server_json_data_array, $url);
                break;
            case "resumeapprove";
            ...
            if ($table_name == "subcategories") {
                ...
                for ($i = 0; $i < $total; $i++) {
                    ...
                    if ($servercountryid)
                        ...
                        $new_client_array[$i]['countryid'] = 0;
                    if(!empty($new_client_array[$i]['stateid'])){
        ...
        switch ($table) {
            case "categories";
                for ($i = 0; $i < $total; $i++) {
                    ...
                    if(!empty($c_cat[0]['id']) && !empty($c_cat[0]['id'])){
            ...
            case "subcategories";
                for ($i = 0; $i < $total; $i++) {
                    ...
                    if(!empty($c_scat[0]['id']) && !empty($c_scat[0]['title'])){
            ...
            case "heighesteducation";
                for ($i = 0; $i < $total; $i++) {
                    ...
                    if(!empty($d_arr[0]['id']) && !empty($d_arr[0]['title'])){
            ...
            case "cities";
                for ($i = 0; $i < $total; $i++) {
                    ...
                    if(!empty($c_address_data[0]['id']) && !empty($c_address_data[0]['name'])){
            ...
            case "salaryrange";
                for ($i = 0; $i < $total; $i++) {
                    ...
                    if(!empty($c_jobsr[0]['id']) && !empty($c_jobsr[0]['rangestart']) && !empty($c_jobsr[0]['rangeend'])){
            ...
            foreach ($client_job_companies AS $company) {
                ...
                if ($rejected != 1) {
                    ...
                    $server_company_data_array = array('uid' => $company['uid'], 'category' => $comp_cat_s_id, 'name' => $company['name'], 'alias' => $company['alias'],
                        ...
                        'company_id' => $company['id'],'endgolddate' => $company['endgolddate'], 'endfeatureddate' => $company['endfeatureddate'],'notifications'=> $company['notifications'],
            ...
            foreach ($client_job_jobs AS $job) {
                ...
                if ($rejected != 1) {
                    ...
                    $server_job_data_array = array('uid' => $job['uid'], 'companyid' => $job['companyid'], 'title' => $job['title'], 'alias' => $job['alias'],
                        ...
                        'job_id' => $job['id'],'notifications'=>$job['notifications'],'joblink'=>$job['joblink'],'jobapplylink'=>$job['jobapplylink'],
                        'authkey' => $auth_key, 'siteurl' => $this->_siteurl);
        ...
        if ((is_array($client_job_resume)) AND ( !empty($client_job_resume))) {
            foreach ($client_job_resume AS $resume) {
                ...
                if ($rejected != 1) {
                    ...
                    if ($resume['nationality']) $resume['nationality'] = $this->getServerid('countries', $resume['nationality']);
                    else $resume['nationality'] = 0;

                    if ($resume['job_category']) $resume['job_category'] = $this->getServerid('categories', $resume['job_category']);
                    else $resume['job_category'] = 0;

                    if ($resume['jobsalaryrange']) $resume['jobsalaryrange'] = $this->getServerid('salaryrange', $resume['jobsalaryrange']);
                    else $resume['jobsalaryrange'] = 0;

                    if ($resume['jobtype']) $resume['jobtype'] = $this->getServerid('jobtypes', $resume['jobtype']);
                    else $resume['jobtype'] = 0;

                    if ($resume['heighestfinisheducation']) $resume['heighestfinisheducation'] = $this->getServerid('heighesteducation', $resume['heighestfinisheducation']);
                    else $resume['heighestfinisheducation'] = 0;

                    //Addresses sends to server
                    $addresses = array();
                    if(!empty($resume['resume_addresses'])){
                        foreach($resume['resume_addresses'] AS $address){
                            $address['address_country'] = $this->getServerid('countries',$address['address_country']);
                            $address['address_state'] = $this->getServerid('states',$address['address_state']);
                            $address['address_city'] = $this->getServerid('cities',$address['address_city']);
                            $addresses[] = $address;
                        }
                    }
                    $resume['resume_addresses'] = $addresses;
                    
                    //Employers sends to server
                    $employers = array();
                    if(!empty($resume['resume_employers'])){
                        foreach($resume['resume_employers'] AS $employer){
                            $employer['employer_country'] = $this->getServerid('countries',$employer['employer_country']);
                            $employer['employer_state'] = $this->getServerid('states',$employer['employer_state']);
                            $employer['employer_city'] = $this->getServerid('cities',$employer['employer_city']);
                            $employers[] = $employer;
                        }
                    }
                    $resume['resume_employers'] = $employers;

                    //Institutes sends to server
                    $institutes = array();
                    if(!empty($resume['resume_institutes'])){
                        foreach($resume['resume_institutes'] AS $institute){
                            $institute['institute_country'] = $this->getServerid('countries',$institute['institute_country']);
                            $institute['institute_state'] = $this->getServerid('states',$institute['institute_state']);
                            $institute['institute_city'] = $this->getServerid('cities',$institute['institute_city']);
                            $institutes[] = $institute;
                        }
                    }
                    $resume['resume_institutes'] = $institutes;

                    //References sends to server
                    $references = array();
                    if(!empty($resume['resume_references'])){
                        foreach($resume['resume_references'] AS $reference){
                            $reference['reference_country'] = $this->getServerid('countries',$reference['reference_country']);
                            $reference['reference_state'] = $this->getServerid('states',$reference['reference_state']);
                            $reference['reference_city'] = $this->getServerid('cities',$reference['reference_city']);
                            $references[] = $reference;
                        }
                    }
                    $resume['resume_references'] = $references;

                    if ($resume['currencyid']) $resume['currencyid'] = $this->getServerid('currencies', $resume['currencyid']);
                    else $resume['currencyid'] = 0;

                    if ($resume['job_subcategory']) $resume['job_subcategory'] = $this->getServerid('subcategories', $resume['job_subcategory']);
                    else $resume['job_subcategory'] = 0;
                    ...
                    foreach($resume['resume_files'] AS $file){
                        $file['path'] = JURI::root() . $config['data_directory'] . "/data/jobseeker/resume_" . $resume['id'] . "/resume/" . $file['filename'];
                        $files[] = $file;
                    }
                    $resume['resume_files'] = $files;
        ...
        foreach ($is_update_serverstatus_ids AS $data) {
            ...
            switch ($table) {
                ...
                case "jobalertsetting";
                    ...
                    break;
                case "resume":
                    foreach($data->resumefile_id AS $file){
                        $query = "UPDATE `#__js_job_resumefiles` SET serverstatus = 'ok', serverid=" . $file->server_id . " WHERE id = " . $file->client_id;
                        $db->setQuery($query);
                        if (!$db->query()) {
                            $error[] = $this->_db->getErrorMsg();
                        }
                    }
                    foreach($data->resumereference_id AS $reference){
                        $query = "UPDATE `#__js_job_resumereferences` SET serverstatus = 'ok', serverid=" . $reference->server_id . " WHERE id = " . $reference->client_id;
                        $db->setQuery($query);
                        if (!$db->query()) {
                            $error[] = $this->_db->getErrorMsg();
                        }
                    }
                    foreach($data->resumelanguage_id AS $language){
                        $query = "UPDATE `#__js_job_resumelanguages` SET serverstatus = 'ok', serverid=" . $language->server_id . " WHERE id = " . $language->client_id;
                        $db->setQuery($query);
                        if (!$db->query()) {
                            $error[] = $this->_db->getErrorMsg();
                        }
                    }
                    foreach($data->resumeinstitute_id AS $institute){
                        $query = "UPDATE `#__js_job_resumeinstitutes` SET serverstatus = 'ok', serverid=" . $institute->server_id . " WHERE id = " . $institute->client_id;
                        $db->setQuery($query);
                        if (!$db->query()) {
                            $error[] = $this->_db->getErrorMsg();
                        }
                    }
                    foreach($data->resumeemployer_id AS $employer){
                        $query = "UPDATE `#__js_job_resumeemployers` SET serverstatus = 'ok', serverid=" . $employer->server_id . " WHERE id = " . $employer->client_id;
                        $db->setQuery($query);
                        if (!$db->query()) {
                            $error[] = $this->_db->getErrorMsg();
                        }
                    }
                    foreach($data->resumeaddress_id AS $address){
                        $query = "UPDATE `#__js_job_resumeaddresses` SET serverstatus = 'ok', serverid=" . $address->server_id . " WHERE id = " . $address->client_id;
                        $db->setQuery($query);
                        if (!$db->query()) {
                            $error[] = $this->_db->getErrorMsg();
                        }
    ...
    function storeResumeSectionSharing($section_data) {
        $section_data['siteurl'] = $this->_siteurl;
        $server_resumesection_data_array = $section_data;
        $server_data_array = array('data' => $server_resumesection_data_array);
        if (!empty($server_data_array)) {
            $fortask = $section_data['task'];
    ...
    function storeResumeSharing($data) {
        ...
        if ($rejected != 1) {
            ...
            if ($data->nationality) $data->nationality = $this->getServerid('countries', $data->nationality);
            else $data->nationality = 0;

            if ($data->job_category) $data->job_category = $this->getServerid('categories', $data->job_category);
            else $data->job_category = 0;

            if ($data->jobsalaryrange) $data->jobsalaryrange = $this->getServerid('salaryrange', $data->jobsalaryrange);
            else $data->jobsalaryrange = 0;

            if ($data->jobtype) $data->jobtype = $this->getServerid('jobtypes', $data->jobtype);
            else $data->jobtype = 0;

            if ($data->heighestfinisheducation) $data->heighestfinisheducation = $this->getServerid('heighesteducation', $data->heighestfinisheducation);
            else $data->heighestfinisheducation = 0;

            //Addresses sends to server
            $resume_addresses = $this->getSharingAdminModel('resume')->getResumeSectionByResumeidAndSectionName($data->id,'resumeaddresses');
            $resumeaddresses = array();
            if(!empty($resume_addresses)){
                foreach($resume_addresses AS $address){
                    $resume_address['address'] = $address->address;
                    $resume_address['address_zipcode'] = $address->address_zipcode;
                    $resume_address['longitude'] = $address->longitude;
                    $resume_address['latitude'] = $address->latitude;
                    $resume_address['address_country'] = $this->getServerid('countries',$address->address_country);
                    $resume_address['address_state'] = $this->getServerid('states',$address->address_state);
                    $resume_address['address_city'] = $this->getServerid('cities',$address->address_city);
                    $resume_address['created'] = $address->created;
                    $resume_address['last_modified'] = $address->last_modified;
                    $resumeaddresses[] = $resume_address;
                }
            }
            $data->resume_addresses = $resumeaddresses;
            
            //Employers sends to server
            $resume_employers = $this->getSharingAdminModel('resume')->getResumeSectionByResumeidAndSectionName($data->id,'resumeemployers');
            $resumeemployers = array();
            if(!empty($resume_employers)){
                foreach($resume_employers AS $employer){
                    $resume_employer['employer'] = $employer->employer;
                    $resume_employer['employer_position'] = $employer->employer_position;
                    $resume_employer['employer_resp'] = $employer->employer_resp;
                    $resume_employer['employer_pay_upon_leaving'] = $employer->employer_pay_upon_leaving;
                    $resume_employer['employer_supervisor'] = $employer->employer_supervisor;
                    $resume_employer['employer_from_date'] = $employer->employer_from_date;
                    $resume_employer['employer_to_date'] = $employer->employer_to_date;
                    $resume_employer['employer_leave_reason'] = $employer->employer_leave_reason;
                    $resume_employer['employer_country'] = $this->getServerid('countries',$employer->employer_country);
                    $resume_employer['employer_state'] = $this->getServerid('states',$employer->employer_state);
                    $resume_employer['employer_city'] = $this->getServerid('cities',$employer->employer_city);
                    $resume_employer['employer_zip'] = $employer->employer_zip;
                    $resume_employer['employer_phone'] = $employer->employer_phone;
                    $resume_employer['employer_address'] = $employer->employer_address;
                    $resume_employer['created'] = $employer->created;
                    $resume_employer['last_modified'] = $employer->last_modified;
                    $resumeemployers[] = $resume_employer;
                }
            }
            $data->resume_employers = $resumeemployers;

            //Institutes sends to server
            $resume_institutes = $this->getSharingAdminModel('resume')->getResumeSectionByResumeidAndSectionName($data->id,'resumeinstitutes');
            $resumeinstitutes = array();
            if(!empty($resume_institutes)){
                foreach($resume_institutes AS $institute){
                    $resume_institute['institute'] = $institute->institute;
                    $resume_institute['institute_address'] = $institute->institute_address;
                    $resume_institute['institute_certificate_name'] = $institute->institute_certificate_name;
                    $resume_institute['institute_study_area'] = $institute->institute_study_area;
                    $resume_institute['institute_country'] = $this->getServerid('countries',$institute->institute_country);
                    $resume_institute['institute_state'] = $this->getServerid('states',$institute->institute_state);
                    $resume_institute['institute_city'] = $this->getServerid('cities',$institute->institute_city);
                    $resume_institute['created'] = $institute->created;
                    $resume_institute['last_modified'] = $institute->last_modified;
                    $resumeinstitutes[] = $resume_institute;
                }
            }
            $data->resume_institutes = $resumeinstitutes;

            //Languages sends to server
            $resume_languages = $this->getSharingAdminModel('resume')->getResumeSectionByResumeidAndSectionName($data->id,'resumelanguages');
            $resumelanguages = array();
            if(!empty($resume_languages)){
                foreach($resume_languages AS $language){
                    $resume_language['language'] = $language->language;
                    $resume_language['language_reading'] = $language->language_reading;
                    $resume_language['language_writing'] = $language->language_writing;
                    $resume_language['language_understanding'] = $language->language_understanding;
                    $resume_language['language_where_learned'] = $language->language_where_learned;
                    $resume_language['created'] = $language->created;
                    $resume_language['last_modified'] = $language->last_modified;
                    $resumelanguages[] = $resume_language;
                }
            }
            $data->resume_languages = $resumelanguages;

            //References sends to server
            $resume_references = $this->getSharingAdminModel('resume')->getResumeSectionByResumeidAndSectionName($data->id,'resumereferences');
            $resumereferences = array();
            if(!empty($resume_references)){
                foreach($resume_references AS $reference){
                    $resume_reference['reference'] = $reference->reference;
                    $resume_reference['reference_name'] = $reference->reference_name;
                    $resume_reference['reference_country'] = $this->getServerid('countries',$reference->reference_country);
                    $resume_reference['reference_state'] = $this->getServerid('states',$reference->reference_state);
                    $resume_reference['reference_city'] = $this->getServerid('cities',$reference->reference_city);
                    $resume_reference['reference_zipcode'] = $reference->reference_zipcode;
                    $resume_reference['reference_address'] = $reference->reference_address;
                    $resume_reference['reference_phone'] = $reference->reference_phone;
                    $resume_reference['reference_relation'] = $reference->reference_relation;
                    $resume_reference['reference_years'] = $reference->reference_years;
                    $resume_reference['created'] = $reference->created;
                    $resume_reference['last_modified'] = $reference->last_modified;
                    $resumereferences[] = $resume_reference;
                }
            }
            $data->resume_references = $resumereferences;

            if ($data->currencyid) $data->currencyid = $this->getServerid('currencies', $data->currencyid);
            else $data->currencyid = 0;

            if ($data->job_subcategory) $data->job_subcategory = $this->getServerid('subcategories', $data->job_subcategory);
            else $data->job_subcategory = 0;
    ...
    function storeCompanySharing($data) {
        ...
        if ($rejected != 1) {
            ...
            $server_company_data_array = array('uid' => $data->uid, 'category' => $cat_s_id, 'name' => $data->name, 'alias' => $data->alias,
                ...
                'company_id' => $data->company_id,'endgolddate' => date('Y-m-d H:i:s',strtotime($data->endgolddate)), 'endfeatureddate' => date('Y-m-d H:i:s',strtotime($data->endfeatureddate)),'notifications'=>$data->notifications,
                'authkey' => $data->authkey, 'id' => $data->id, 'siteurl' => $this->_siteurl);
    ...
    function storeJobSharing($data) {
        ...
        if ($rejected != 1) {
            ...
            $server_job_data_array = array('uid' => $data->uid, 'companyid' => $data->companyid, 'title' => $data->title, 'alias' => $data->alias,
                ...
                'job_id' => $data->job_id,'notifications'=> $data->notifications,'joblink'=> $data->joblink,'jobapplylink'=> $data->jobapplylink,
                'authkey' => $data->authkey, 'id' => $data->id, 'siteurl' => $this->_siteurl);
    ...
    function getAllResumeSynToServer() {
        ...
        $db->setQuery($query);
        $this->_application = $db->loadObjectList();
        foreach($this->_application AS $resume){
            $resume->resume_addresses = $this->getSharingAdminModel('resume')->getResumeSectionByResumeidAndSectionName($resume->id,'resumeaddresses');
            $resume->resume_employers = $this->getSharingAdminModel('resume')->getResumeSectionByResumeidAndSectionName($resume->id,'resumeemployers');
            $resume->resume_institutes = $this->getSharingAdminModel('resume')->getResumeSectionByResumeidAndSectionName($resume->id,'resumeinstitutes');
            $resume->resume_languages = $this->getSharingAdminModel('resume')->getResumeSectionByResumeidAndSectionName($resume->id,'resumelanguages');
            $resume->resume_references = $this->getSharingAdminModel('resume')->getResumeSectionByResumeidAndSectionName($resume->id,'resumereferences');
            $resume->resume_files = $this->getSharingAdminModel('resume')->getResumeSectionByResumeidAndSectionName($resume->id,'resumefiles');
        }
        return $this->_application;
    }
    ...
    function performTaskJobSharingService($jsondata, $url) {
        ...
        $clientInfo['cms'] = 'joomla';
        if(JFactory::getApplication()->IsAdmin())
            $clientInfo['clientversion'] = $this->getJSModel('configuration')->getConfigValue('versioncode');
        else
            $clientInfo['clientversion'] = $this->getJSModel('configurations')->getConfigValue('versioncode');
        $jsonclientInfo = json_encode($clientInfo);
        curl_setopt($ch, CURLOPT_POSTFIELDS, array('data' => $jsondata,'clientInfo' => $jsonclientInfo));
    ...
    function DefaultListAddressDataSharing($data, $val, $hasstate) {
        ...
        if ($data == 'defaultsharingstate') {  // states
            ...
            if (empty($result)) {
                ...
                $return_value .= "<option value='0'>" . JText::_('Select State') . "</option>\n";
            ...
            if (empty($result)) {
                ...
                $return_value .= "<option value='0'>" . JText::_('Select City') . "</option>\n";
