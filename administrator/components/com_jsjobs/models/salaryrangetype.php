...
class JSJobsModelSalaryrangetype extends JSModel {
    ...
    function getSalaryRangeTypebyId($c_id) {
        if (is_numeric($c_id) == false)
            return false;
        $db = JFactory::getDBO();
        $query = "SELECT id,title,status,isdefault,ordering FROM #__js_job_salaryrangetypes WHERE id = " . $c_id;
    ...
    function getAllSalaryRangeType($searchtitle, $searchstatus, $limitstart, $limit) {
        $db = JFactory::getDBO();
        $fquery="";
        $clause=" WHERE ";
        if($searchtitle){
        ...
        if ($total <= $limitstart)
            $limitstart = 0;
        $query = "SELECT id,title,status,isdefault,ordering FROM #__js_job_salaryrangetypes";
        $query .=$fquery;
    ...
    function storeSalaryRangeType() {
        ...
        if ($data['id'] == '') { // only for new
            ...
            if($data['status'] == 0){
                $return_var = $this->getJSModel('common')->canUnpublishRecord($data['id'],'salaryrangetypes');
                if($return_var=='no'){
                    $data['status'] = 1;
                }
            }
