...
function modChrome_jsnmodule( $module, &$params, &$attribs ) {
	...
	$moduleClass = $params->get( 'moduleclass_sfx' );
		...
		if (preg_match('/^(.+)?(fa fa-[^\s]+)(.+)?$/', $params->get( 'moduleclass_sfx' ), $match))
		{
			if ($module->showtitle) {
				...
				$moduleTitleOuput = '<span class="jsn-moduleicon "><i class="' . $match[2] . '"></i></span>';
			}
			$moduleClass = $match[1] . ' ' . $match[3];
		}
		if (preg_match('/^(.+)?(jsn-icon-[^\s]+)(.+)?$/', $params->get( 'moduleclass_sfx' ), $match))
		{
			if ($module->showtitle) {
				$moduleTitleOuput = '<span class="jsn-moduleicon "><i class="' . $match[2] . '"></i>'.$module->title.'</span>';
			}
	...
	if (strpos($moduleClass, 'display-dropdown') !== false && ($module->position == 'mainmenu' || $module->position == 'toolbar' || $module->position == 'top')) {
			if ($module->showtitle) {
				echo '<h3 class="jsn-moduletitle">' . $moduleTitleOuput . '</h3>';
			}
			else {
				echo '<h3 class="jsn-moduletitle no-title">' . $moduleTitleOuput . '</h3>';
			}
	} else {
		if ($module->showtitle) {
		echo '<h3 class="jsn-moduletitle">'.$moduleTitleOuput.'</h3>';
